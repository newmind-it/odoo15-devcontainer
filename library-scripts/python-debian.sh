...
if ! type pipx > /dev/null 2>&1; then
    pip3 install --ignore-installed --user pipx 2>&1
    /tmp/pip-tmp/bin/pipx install --pip-args= --ignore-installed pipx
    pipx_path="/tmp/pip-tmp/bin/"
fi
for util in ${DEFAULT_UTILS[@]}; do
    if ! type ${util} > /dev/null 2>&1; then
        ${pipx_path}pipx install --system-site-packages --pip-args '--ignore-installed --force-reinstall' ${util}
    else
